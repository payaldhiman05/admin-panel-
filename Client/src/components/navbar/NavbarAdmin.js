					...
					*/}
					<Flex me={openSidebar ? "" : "5"} mx={openSidebar ? "14" : "1"} display={{ sm: "none", xl: "flex" }}>
						{largeLogo && largeLogo[0]?.logoLgImg || largeLogo && largeLogo[0]?.logoSmImg ? <Image
							...
							userSelect="none"
							my={2}
						/> : <Heading my={4} style={{ width: openSidebar ? "165px" : "60px", height: '31px', objectFit: "contain", fontSize:'27px', verticalAlign:'middle'}}
							cursor={"pointer"} onClick={() => !props.from && setOpenSidebar(!openSidebar)} userSelect={"none"}>{openSidebar === true ? "SwapSignal " : "SS"}</Heading>}
					</Flex>
					...
					>
						{largeLogo && largeLogo[0]?.logoLgImg ? <Image
							...
							my={2}
						/> : <Heading my={4}
							cursor={"pointer"} userSelect={"none"}>{openSidebar === true ? "SwapSignal " : "SS"}</Heading>}
					</Link>
